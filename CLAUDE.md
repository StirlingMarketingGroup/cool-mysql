...
After a durable write through the writer pool (`Exec`, `Insert`, `Upsert`, or a committed writer `Tx` / `RunInTx`), `Select` / `Exists` / `Count` issued on the same `*Database` (including a `Clone()` — the write marker is a shared pointer) within `ReadYourWritesWindow` (default 5s, env `COOL_READ_YOUR_WRITES_WINDOW`; `0` disables) are routed to the `Writes` pool and skip the query cache, so a lagging Aurora replica cannot hide those rows. `SelectWrites` / `ExistsWrites` still always hit Writes, cache included, for callers that want the writer regardless of recency. The marker covers cool-mysql methods only: a caller that uses `db.Writes.ExecContext` or `tx.Tx.ExecContext` directly is outside the guarantee. Provenance is stated by the API (`Insert`/`Exec`/`BeginTx` = writer, `InsertReads`/`BeginReadsTx` = reader); `Inserter.SetExecutor` drops provenance, so statements through a custom executor never mark.
