...
`Select` / `Exists` / `Count` normally use the `Reads` pool (a replica in dual-host setups). Read-your-writes is a property of a **session**: `db.NewSession()` returns a copy (same pools, caches, settings) with its own write marker, and after a successful write through cool-mysql on that session (`Exec`, `Insert`, `Upsert`, or a committed writer transaction), its reads go to `Writes` for `ReadYourWritesWindow` (default 5 seconds; env `COOL_READ_YOUR_WRITES_WINDOW` as integer seconds; `0` disables) and skip the query cache. Make one session per request/unit of work; a raw constructed `Database` has no marker and never pins, and `Clone()` shares its source's marker. Use `SelectWrites` / `ExistsWrites` to always hit the writer regardless of recency. Direct `db.Writes` / `tx.Tx` use is not tracked. Provenance is stated by the API (`Insert`/`Exec`/`BeginTx` = writer, `InsertReads`/`BeginReadsTx` = reader); `Inserter.SetExecutor` drops provenance, so statements through a custom executor never mark.
